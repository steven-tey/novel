...
async function startTypeChecking({ cacheDir , config , dir , ignoreESLint , nextBuildSpan , pagesDir , runLint , shouldLint , telemetry , appDir  }) {
    ...
    if (typeCheckingAndLintingSpinnerPrefixText) {
        typeCheckingAndLintingSpinner = (0, _spinner.default)({
            prefixText: `${_log.prefixes.info} ${typeCheckingAndLintingSpinnerPrefixText}`
        });
    }
    ...
    try {
        const [[verifyResult, typeCheckEnd]] = await Promise.all([
            ...
            shouldLint && nextBuildSpan.traceChild("verify-and-lint").traceAsyncFn(async ()=>{
                var _config_eslint;
                await (0, _verifyAndLint.verifyAndLint)(dir, eslintCacheDir, (_config_eslint = config.eslint) == null ? void 0 : _config_eslint.dirs, config.experimental.workerThreads, telemetry, !!appDir);
            })
        ]);
