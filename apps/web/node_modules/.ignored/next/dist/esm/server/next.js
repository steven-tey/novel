...
import { PHASE_DEVELOPMENT_SERVER, SERVER_DIRECTORY } from "../shared/lib/constants";
...
export class NextServer {
    ...
    async getServerRequestHandler() {
        ...
        if (!this.reqHandlerPromise) {
            this.reqHandlerPromise = this.getServer().then((server)=>getTracer().wrap(NextServerSpan.getServerRequestHandler, server.getRequestHandler().bind(server)));
        }
                ...
                switch(propKey){
                    case "prepare":
                        return async ()=>{
                            ...
                            // in the dist folder.
                            const distDir = process.env.NEXT_RUNTIME === "edge" ? config.distDir : join(dir, config.distDir);
                            const serverDistDir = join(distDir, SERVER_DIRECTORY);
                            const hasAppDir = !!findDir(options.dev ? dir : serverDistDir, "app");
                            if (hasAppDir) {
