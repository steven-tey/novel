...
class Router {
        ...
        if (process.env.__NEXT_CLIENT_ROUTER_FILTER_ENABLED) {
            ...
            for (const curAs of [
                ...
                if (curAs) {
                    ...
                    if (asNoSlash !== removeTrailingSlash(new URL(this.asPath, "http://n").pathname)) {
        ...
        try {
            ...
            if ((data == null ? void 0 : (_data_effect2 = data.effect) == null ? void 0 : _data_effect2.type) === "rewrite") {
                ...
                if (!isQueryUpdating || pages.includes(resolvedRoute)) {
                    ...
                    resolvedAs = removeBasePath(normalizeLocalePath(data.effect.parsedAs.pathname, this.locales).pathname);
   ...
   */ async prefetch(url, asPath, options) {
        ...
        if (process.env.__NEXT_HAS_REWRITES && asPath.startsWith("/")) {
            ...
            const rewritesResult = resolveRewrites(addBasePath(addLocale(asPath, this.locale), true), pages, rewrites, parsed.query, (p)=>resolveDynamicRoute(p, pages), this.locales);
            if (rewritesResult.externalDest) {
                return;
            }
            if (!isMiddlewareMatch) {
                resolvedAs = removeLocale(removeBasePath(rewritesResult.asPath), this.locale);
            }
            if (rewritesResult.matchedPage && rewritesResult.resolvedHref) {
                // if this directly matches a page we need to update the href to
                // allow the correct page chunk to be loaded
