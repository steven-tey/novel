...
Since a dialog is a portal, it isn't possible to scroll the command list when it's opened in a dialog since both are attached to the body but the default behaviour Radix's Dialog is to lock the scroll of the body when open.
